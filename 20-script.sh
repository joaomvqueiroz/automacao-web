...
# =================================================================
# Script: Apache HTTPS + SSL + DuckDNS (IPv4 & IPv6, fallback automático)
# Objetivo: Configurar DuckDNS, Apache, SSL e Certbot com fallback automático
# =================================================================
...
echo -e "${YELLOW}--- Iniciando configuração Apache + HTTPS + DuckDNS (IPv4 & IPv6) ---${NC}"

# =================================================================
# 1. Solicitar dados do usuário
# =================================================================
log_info "1. Solicitar informações"
...
# =================================================================
# 2. Pré-verificação de ferramentas
# =================================================================
log_info "2. Verificação pré-requisitos"
if ! ps -ef | grep -q 'cr[o]n'; then
    echo "⚠️ ATENÇÃO: cron não está ativo. Instale e inicie o cron."
else
    echo "✅ Cron detectado"
fi

if ! command -v curl &>/dev/null; then
    echo "❌ curl não encontrado. Instale com: sudo dnf install curl -y"
    exit 1
else
...
# =================================================================
# 3. Detectar IPs públicos
# =================================================================
log_info "3. Detectando IPs públicos"
...
# =================================================================
# 4. Configuração DuckDNS
# =================================================================
log_info "4. Configuração DuckDNS"
sudo mkdir -p "$DUCK_DIR" && sudo chmod 700 "$DUCK_DIR"

sudo bash -c "cat > ${DUCK_SCRIPT}" <<EOF
#!/bin/bash
# Atualização DuckDNS para ${APACHE_DOMAIN}
...
if grep -q "OK" "$DUCK_LOG" 2>/dev/null; then
    ...
    echo -e "${YELLOW}⚠️ Verifique ${DUCK_LOG} para status da atualização.${NC}"
fi
echo "--------------------------------------------------------"

# =================================================================
# 5. Instalar Apache, mod_ssl e Certbot
# =================================================================
log_info "5. Instalando pacotes Apache/SSL/Certbot"

# Tenta EPEL + pacotes padrão
sudo dnf install epel-release -y
sudo dnf install httpd mod_ssl openssl certbot python3-certbot-apache -y

# Se Certbot não existe, tenta Snap
...
# =================================================================
log_info "6. Abrir portas 80 e 443"
...
# =================================================================
# 7. Certificados temporários (self-signed)
# =================================================================
log_info "7. Certificados temporários"
...
echo -e "${GREEN}✔️ Certificado temporário criado${NC}"
echo "--------------------------------------------------------"

# =================================================================
# 8. Certificados Self-Signed
# =================================================================
log_info "8. Certificados self-signed"
...
sudo openssl req -x509 -nodes -days 365 \
  ...
  -subj "/C=PT/ST=Lisboa/L=Lisboa/O=${DUCK_DOMAIN}/OU=IT/CN=${APACHE_DOMAIN}"
echo -e "${GREEN}✔️ Certificados simulados criados em ${LE_DIR}${NC}"
echo "--------------------------------------------------------"

# =================================================================
# 9. Configuração VirtualHosts Apache
# =================================================================
log_info "9. VirtualHosts Apache"
...
EOF_SSL
echo -e "${GREEN}✔️ VirtualHosts criados${NC}"
echo "--------------------------------------------------------"

# =================================================================
# 10. Reiniciar Apache
# =================================================================
log_info "10. Reiniciar Apache"

sudo apachectl configtest
sudo systemctl restart httpd
sudo systemctl enable httpd
echo -e "${GREEN}✔️ Apache reiniciado e habilitado${NC}"
echo "--------------------------------------------------------"

# =================================================================
# 11. Emitir certificado Let’s Encrypt com fallback
# =================================================================
log_info "11. Emitir certificado Let’s Encrypt (IPv4 & IPv6, fallback)"
